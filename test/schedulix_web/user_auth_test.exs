defmodule SchedulixWeb.UserAuthTest do
  use SchedulixWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Schedulix.Accounts
  alias Schedulix.Accounts.Scope
  alias SchedulixWeb.UserAuth

  import Schedulix.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SchedulixWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SchedulixWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: SchedulixWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SchedulixWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
